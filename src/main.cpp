...
void setup() {
  ...
  // emg_stat2 = emg_stat_sum_2 / 1000.0;
...
void loop() {
  ...
  // float emg_bp2 = emaFilt2.BPF(emg_raw2, f_c_bplow, f_c_bphigh, f_s); // bandpassed emg signal between 50 Hz and 150 Hz
  ...
  // float emg_evlp2 =  pow(emaFilt2.LPF(abs(emg_bp2), f_c_lp, f_s), 2); // take magnitude, lowpass, then square
